...
- **TLS (added 2026-08-13):** two nginx configs exist side by side — `frontend/nginx.conf` (active, plain HTTP on `:80`, what the Dockerfile currently builds) and `frontend/nginx.https.conf` (on the shelf, not wired into the Dockerfile yet). `[ASSUMPTION]`, user's explicit call: no self-signed-cert bootstrapping and no `certbot`/proxy service added to `docker-compose.yml` — the user gets the real certificate for their domain out-of-band and will swap the two-line Dockerfile `COPY` + mount the cert files + open `443:443` themselves (instructions live in `nginx.https.conf`'s header comment) once that's in hand. `docker-compose.yml`'s frontend port changed from the dev-only `5174:80` to `80:80` for this reason too — a real domain's plain-HTTP-before-HTTPS phase, and any future ACME HTTP-01 validation, both need the standard port, not a project-local one.
