...
type PendingDecision = { request: PermissionRequestItem; decision: "approve" | "deny" };
...
export function PermissionRequestsPage() {
  ...
  async function load() {
    const res = await api.get<PermissionRequestItem[]>("/permission-requests", { params: { status } });
    setItems(res.data);
  }
  ...
  async function handleConfirm() {
    ...
    setSubmitting(true);
    try {
      await api.post(`/permission-requests/${pendingDecision.request.id}/${pendingDecision.decision}`, {
        note: note.trim() || null,
      });
      toast.success(pendingDecision.decision === "approve" ? "Ruxsat berildi." : "Rad etildi.");
  ...
  return (
    <AppShell title="Ruxsat so'rovlari">
      ...
      <div className="rounded-lg border">
        <Table>
          ...
          <TableBody>
            {(items ?? []).map((r) => (
              <TableRow key={r.id}>
                <TableCell className="font-medium whitespace-nowrap">{r.requested_by}</TableCell>
      ...
      <AlertDialog open={pendingDecision !== null} onOpenChange={(open) => !open && setPendingDecision(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>
              {pendingDecision?.decision === "approve" ? "Ruxsat berish" : "Rad etish"} —{" "}
